...
namespace global_motion_planner
{
   ...
   */
  GA::GA(int nx, int ny, double resolution, double origin_x, double origin_y, int n_genets,int n_inherited, int chromLength , double p_select, double p_crs, double p_mut, double obs_factor ,int max_speed,int initposmode ,bool pub_genets,int max_iter)
    : nx_(nx)
    , ny_(ny)
    , ns_(nx*ny)
   ...
   */
  bool GA::plan(const unsigned char* global_costmap, const std::pair<int, int>& start, const std::pair<int, int>& goal, std::vector< std::pair<int, int>>& path)
  {
    ...
    if (!initial_point.empty()) 
    {
      ...
      for (int p = 1; p <initial_point.size(); ++p) 
      {
  ...
  int GA::grid2Index(int x, int y){return x + nx_ * y;}
